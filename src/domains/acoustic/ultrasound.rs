...
impl PhasedArray {
    ...
    pub fn focus_at(&mut self, target: &Coord3D) -> Vec<Scalar> {
        let distances: Vec<Scalar> = self
            ...
            .collect();
        let max_dist = crate::core::compute::vector::vec_max_abs(&distances).unwrap_or(0.0);
    ...
    pub fn beam_pattern(&self, theta_range: (Scalar, Scalar, usize)) -> Vec<Scalar> {
        ...
        for i in 0..n {
            ...
            for (j, e) in self.elements.iter().enumerate() {
                let phase = k * (e.x * theta.sin() + e.y * theta.cos())
                    + 2.0 * std::f64::consts::PI * self.frequency * self.delays[j];
