...
pub fn eddy_current_loss(freq: Scalar, b_peak: Scalar, thickness: Scalar, conductivity: Scalar, volume: Scalar) -> Scalar {
    if freq <= 0.0 || thickness <= 0.0 { return 0.0; }
...
pub fn radar_range_eq(pt: Scalar, gt: Scalar, gr: Scalar, sigma: Scalar, lambda: Scalar, snr_min: Scalar, losses: Scalar) -> Scalar {
    if snr_min <= 0.0 || lambda <= 0.0 { return 0.0; }
...
pub fn shielding_effectiveness(freq: Scalar, _material: &str, thickness: Scalar, conductivity: Scalar, mu_r: Scalar) -> Scalar {
    ...
    let correction = if absorption > 10.0 { 0.0 } else { -10.0 * f64::log10(1.0 - 10.0_f64.powf(-absorption / 10.0)) };
...
mod tests {
    ...
    fn test_joule_heating() {
        assert!((joule_heating(10.0, 0.1) - 10.0).abs() < 1e-10);
    }
