...
impl ReentryTrajectory {
    pub fn new() -> Self {
        Self { states: Vec::new(), ballistic_coefficient: 100.0 }
    }
    pub fn propagate(&mut self, dt: Scalar, t_end: Scalar) -> Result<(), String> {
        if dt <= 0.0 { return Err(String::from("bad dt")); }
        ...
        for _ in 0..=(t_end / dt) as usize {
            ...
            self.states.push(ReentryState { altitude: a, velocity: v, heat_flux: q, deceleration: de });
            v -= de * dt;
            a -= v * dt;
            if a <= 0.0 { break; }
        }
    ...
    pub fn max_heat_flux(&self) -> Scalar {
        self.states.iter().map(|s| s.heat_flux).fold(0.0_f64, f64::max)
    }
}
impl Default for ReentryTrajectory { fn default() -> Self { Self::new() } }
