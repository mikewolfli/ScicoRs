...
impl DcMotor {
    pub fn back_emf(&self, omega: Scalar) -> Scalar {
        self.ke * omega
    }
    pub fn torque(&self, i_a: Scalar) -> Scalar {
        self.kt * i_a
    }
    ...
    pub fn steady_state_speed(&self, v_a: Scalar, t_load: Scalar) -> Scalar {
        if self.ke * self.kt + self.b * self.ra <= 0.0 {
            return 0.0;
        }
...
impl StepperMotor {
    pub fn step_angle(&self) -> Scalar {
        360.0 / self.steps_per_rev as Scalar
    }
...
impl InductionMotor {
    ...
    pub fn torque_slip(&self, v_phase: Scalar, slip: Scalar, freq: Scalar) -> Scalar {
        ...
        let x_lr = omega_s * self.lr;
        let x_m = omega_s * self.lm;
        let a = self.rr / slip; // rotor resistance / slip

        // Exact equivalent circuit (stator + rotor + magnetizing branch).
        // Rotor branch (R2/s + jX_lr) in parallel with the magnetizing
        // reactance jX_m:
        //   Zp = jX_m·(a + jX_lr) / (a + j(X_lr + X_m))
        //     = [ X_m²·a + j·(X_m·a² + X_m·X_lr·(X_lr + X_m)) ] / (a² + (X_lr + X_m)²)
        let den = a * a + (x_lr + x_m) * (x_lr + x_m);
        if den <= 1e-30 {
            return 0.0;
        }
        let zp_re = x_m * x_m * a / den;
        let zp_im = (x_m * a * a + x_m * x_lr * (x_lr + x_m)) / den;

        // Total series impedance Z = R_s + jX_ls + Zp.
        let z_re = self.rs + zp_re;
        let z_im = x_ls + zp_im;
        let z2 = z_re * z_re + z_im * z_im;
        if z2 <= 1e-30 {
            0.0
        } else {
            3.0 * v_phase * v_phase * a / (omega_s * z2)
        }
    }
    pub fn breakdown_torque(&self, v_phase: Scalar, freq: Scalar) -> Scalar {
        let slip_max = self.rr
            / f64::sqrt(
                self.rs * self.rs
                    + (freq * 2.0 * std::f64::consts::PI * (self.ls + self.lr)).powi(2),
            );
